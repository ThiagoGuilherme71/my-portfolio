...
function App() {
  ...
  const data = {
    skills: {
      ...
      soft: [
        { name: 'Comunicação', level: 90, color: 'from-pink-500 to-rose-500' },
        { name: 'Liderança', level: 75, color: 'from-purple-500 to-indigo-500' },
        { name: 'Resolução de Problemas', level: 95, color: 'from-green-500 to-emerald-600' },
        { name: 'Adaptabilidade', level: 85, color: 'from-yellow-400 to-orange-500' }
      ]
    },
    experience: [
      {
...
function HeroSection() {
  ...
  return (
    <section id="hero" ref={ref} className="min-h-screen flex items-center justify-center px-4 pt-20 pb-20 relative overflow-hidden">
      <div className="absolute inset-0 z-0">
        <div className="absolute inset-0 z-0">
          <div 
            className="absolute inset-0 bg-cover bg-center"
            style={{ backgroundImage: `url("${import.meta.env.BASE_URL}imgportifolio.jpg")`, backgroundPosition: 'center 30%', filter: 'brightness(0.6) contrast(1.2) sepia(0.4) saturate(1.5)' }}
          />
      ...
      <div className="max-w-6xl w-full relative z-10">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <motion.div initial={{ opacity: 0, x: -30 }} animate={isInView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.8, ease: "easeOut" }}>
            <motion.div className="inline-flex items-center gap-2 bg-white/80 backdrop-blur-md px-4 py-2 rounded-full text-orange-800 font-bold text-sm mb-6 shadow-sm border border-orange-100/50">
              <Icons.MapPin className="w-4 h-4" /> <span>Salvador, Bahia • Brasil 🇧🇷</span>
            </motion.div>

            <motion.h1 className="text-5xl sm:text-7xl font-black mb-6 leading-[1.1] tracking-tight text-white drop-shadow-lg">
              Thiago
              <span className="block text-transparent bg-clip-text bg-gradient-to-r from-yellow-300 via-orange-400 to-amber-400 relative">
                Guilherme
            ...
            <motion.div className="space-y-4 mb-10">
              ...
              <p className="text-lg text-amber-50/90 leading-relaxed max-w-lg font-medium drop-shadow">
                Resolver problemas e dores do mundo real com tecnologia e criatividade é o que me impulsiona. <strong>Vamos melhorar o mundo juntos?!</strong>
              </p>
            </motion.div>

            <div className="flex flex-wrap gap-4">
              <motion.a whileHover={{ scale: 1.03 }} href="#projetos" className="px-8 py-4 bg-gradient-to-r from-orange-500 to-amber-600 text-white font-bold rounded-xl shadow-lg flex items-center gap-2">
                Ver Projetos <Icons.ExternalLink className="w-5 h-5 text-yellow-200" />
              </motion.a>
              <motion.a whileHover={{ scale: 1.03 }} href="#contato" className="px-8 py-4 bg-white/90 backdrop-blur-sm text-orange-900 font-bold rounded-xl shadow-md border-2 border-amber-100">
                Contatos
              </motion.a>
            </div>
          </motion.div>

           <motion.div initial={{ opacity: 0, x: 30 }} animate={isInView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.8, delay: 0.3 }} className="hidden lg:grid grid-cols-2 gap-6 auto-rows-fr">
            {/* Stats Cards (Simplificados para brevidade) */}
            <div className="bg-white/80 backdrop-blur-md p-6 rounded-3xl shadow-xl border border-amber-100/50 flex flex-col justify-center ">
                <Icons.Code2 className="w-10 h-10 text-orange-500 mb-2" />
                <div className="text-2xl font-black text-stone-800">PHP / Laravel</div>
                <div className="text-sm text-stone-600 font-bold">Stack Principal</div>
            </div>
            <div className="bg-white/80 backdrop-blur-md p-6 rounded-3xl shadow-xl border border-amber-100/50 flex flex-col justify-center  mt-8">
                 <Icons.Award className="w-10 h-10 text-blue-500 mb-2" />
                <div className="text-2xl font-black text-stone-800">2+ Anos</div>
                <div className="text-sm text-stone-600 font-bold">Experiência</div>
            </div>
          </motion.div>
        </div>
      </div>
    ...
    <footer className="relative bg-stone-900 text-white pt-20 pb-10 px-6 z-10 overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        <div className="pt-8 border-t border-stone-800 text-center md:text-left flex flex-col md:flex-row justify-between items-center gap-4 text-stone-500 font-medium">
          ...
          <p className="flex items-center gap-2">Feito com <span className="text-red-500">☕</span> e Dendê em Salvador.</p>
        </div>
      </div>
    </footer>
  );
